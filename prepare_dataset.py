...
if __name__ == "__main__":
    ...
    for video_name in tqdm(os.listdir(videos_path)):
        ...
        command = [
            'ffmpeg', '-skip_frame', 'nokey', '-i', f'{video_path}', '-vsync',
            'vfr', '-s', '512x512', '-f', 'image2', '-pix_fmt', 'rgb24',
            os.path.join(video_outdir_frames, '%06d.png')
        ]
        pipe = sp.Popen(command, stdout=sp.PIPE, bufsize=10**8)
        pipe.wait()
        print(f"Extracted frames from {video_path}")
        frames_list = sorted(
            glob.glob(str(video_outdir_frames.joinpath('*.png'))))
        for frame_path in frames_list:
            ...
            image = torch.from_numpy(image).to(device).unsqueeze(0).permute(
                0, 3, 1, 2)
            with torch.no_grad():
